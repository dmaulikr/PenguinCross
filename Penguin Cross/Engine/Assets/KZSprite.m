...
- (void) tverts:(GLfloat *) buffer {
  ...
  if(self.texture.info != nil) {
    GLfloat xstep = 1.f / self.texture.info.width * _cellWidth;
    GLfloat ystep = 1.f / self.texture.info.height * _cellHeight;
    NSInteger hcells = self.texture.info.width / _cellWidth;
    NSInteger vcells = self.texture.info.height / _cellHeight;
