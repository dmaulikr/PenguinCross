...
- (Force *) applyForceToEntity:(GameEntity *) entity {
...
- (void) removeForcesForEntity:(KZEntity *) entity {
  [self.forces removeObjectsMatching: ^(Force *force) {
...
- (void) bounceCollidingEntities:(NSArray *) entities {
  ...
  do {
    ...
    for(Collision *c in collisions) {
      ...
      NSLog(@"%d %f", iteration, victimForce.massAcceleration);
    }
...
- (GameEntity *) firstEntity:(NSArray *) entities touching:(GameEntity *) t {
...
- (Collision *) collisionWith:(GameEntity *) a and: (GameEntity *) b {
...
- (vec3) findCollisionPoint:(GameEntity *) a : (GameEntity *) b {
  ...
  vec3 tl = (vec3){b.origin.x + b.dimensions.x * .5f, b.origin.y - b.dimensions.y * .5f, 0};
  vec3 tr = (vec3){b.origin.x + b.dimensions.x * .5f, b.origin.y + b.dimensions.y * .5f, 0};
  vec3 bl = (vec3){b.origin.x - b.dimensions.x * .5f, b.origin.y - b.dimensions.y * .5f, 0};
  vec3 br = (vec3){b.origin.x - b.dimensions.x * .5f, b.origin.y + b.dimensions.y * .5f, 0};
